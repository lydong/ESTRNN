...
def test(para, logger):
    ...
    elif ds_name == 'REDS_png':
        ds_type = 'valid'
        _test_torch_SR(para, logger, model, ds_type)
    else:
...
def _test_torch_SR(para, logger, model, ds_type):
    PSNR = AverageMeter()
    SSIM = AverageMeter()
    timer = AverageMeter()
    results_register = set()
    H, W = 180, 420
    val_range = 2.0 ** 8 - 1 if para.data_format == 'RGB' else 2.0 ** 16 - 1
    #===== Here to change paths =====
    dataset_path = '/scratch/gilbreth/dong33/REDS_png/valid'
    seq_length = 100
    save_dir = '/scratch/gilbreth/dong33/REDS_png/valid/model_M2/guess_model_epoch200'
    os.makedirs(save_dir, exist_ok=True)
    #========================================================
    seqs = sorted(os.listdir(join(dataset_path, 'valid_sharp')))
    for seq in seqs:
        logger('seq {} image results generating ...'.format(seq))
        suffix = 'png' if para.data_format == 'RGB' else 'tiff'
        start = 0
        ...
        while True:
            ...
            label_seq = []
            for frame_idx in range(start, end):
                blur_img_path = join(dataset_path, 'valid_sharp_bicubic/X4', seq, '{:08d}.{}'.format(frame_idx, suffix))
                sharp_img_path = join(dataset_path, 'valid_sharp', seq, '{:08d}.{}'.format(frame_idx, suffix))
                if para.data_format == 'RGB':
                    blur_img = cv2.imread(blur_img_path).transpose(2, 0, 1)[np.newaxis, ...]
                    gt_img = cv2.imread(sharp_img_path)
                else:
            ...
            for frame_idx in range(para.past_frames, end - start - para.future_frames):
                ...
                deblur_img = np.clip(deblur_img, 0, val_range)
                deblur_img = deblur_img.astype(np.uint8) if para.data_format == 'RGB' else deblur_img.astype(np.uint16)
                deblur_img_path = join(save_dir, seq, '{:08d}_{}.{}'.format(frame_idx + start, para.model.lower(), suffix))
                cv2.imwrite(deblur_img_path, deblur_img)
                if deblur_img_path not in results_register:
                    results_register.add(deblur_img_path)
                    PSNR.update(psnr_calculate(deblur_img, gt_img, val_range=val_range))
