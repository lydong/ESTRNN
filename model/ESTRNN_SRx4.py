...
class RDB_DS(nn.Module):
    def __init__(self, in_channels, growthRate, num_layer, activation='relu'):
        ...
        self.extract_more = conv5x5(in_channels, 2 * in_channels)
        #self.extract_more = conv3x3(in_channels, 2 * in_channels)

    def forward(self, x):
        # x: n,c,h,w
        out = self.rdb(x)
        #out = self.down_sampling(x)
        out = self.extract_more(x)
...
class RDBCell(nn.Module):
    ...
    def forward(self, x, s_last):
        lr_act = self.F_B0(x)
        lr_act = self.F_B1(lr_act)
        lr_act = self.F_B2(lr_act)
        out = torch.cat([lr_act, s_last], dim=1)
        out = self.F_R(out)
        s = self.F_h(out)
...
class Reconstructor(nn.Module):
    def __init__(self, para):
        ...
        self.para = para
        #here i assume scale is 4
        #consider other scale later
        self.scale = para.scale
        self.num_ff = para.future_frames
        self.num_fb = para.past_frames
        self.related_f = self.num_ff + 1 + self.num_fb
        self.n_feats = para.n_features
        self.residual_recons = nn.Sequential(
            ...
            conv3x3((5 * self.n_feats) * (self.related_f), 16 * self.n_feats, stride = 1),
            nn.PixelShuffle(4),
            conv3x3(self.n_feats, 3, stride=1)
        )
        #self.lr_recons = nn.Sequential(
        #    conv3x3(4*self.n_feats, 16 * self.n_feats, stride = 1),
        #    nn.PixelShuffle(4),
        #    conv3x3(self.n_feats, 3, stride=1)
            #nn.ConvTranspose2d((1 + 4) * self.n_feats, 3 * self.n_feats, kernel_size=3, stride=2,
            #                   padding=1, output_padding=1),
            #conv3x3(3*self.n_feats, 3*4, stride = 1),
            #nn.PixelShuffle(2)
        #)
        #self.final_fuse = conv3x3(3, 3, stride=1)
    def forward(self, fused_fmap, lr):
        out = self.residual_recons(fused_fmap) + F.interpolate(lr, scale_factor=4, mode='bilinear', align_corners=False)
        # + self.lr_recons(lr)
...
class Model(nn.Module):
    ...
    def forward(self, x, profile_flag=False):
        ...
        for i in range(self.num_fb, frames - self.num_ff):
            fused_fmap = self.fusion(hs[i - self.num_fb:i + self.num_ff + 1])
            out = self.recons(fused_fmap, lrs[i])
    ...
    def profile_forward(self, x):
        ...
        for i in range(self.num_fb + self.num_ff):
            ...
            lrs.append(torch.randn(*(x[:, i, :, :, :].shape)).to(self.device))
